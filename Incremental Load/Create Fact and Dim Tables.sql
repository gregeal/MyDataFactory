...
create table Dim_product_Cat_SubCat
(
	Prod_Cat_SubCat_Key int identity(1,1) primary key,
	Product_Cat_Code int,
	Product_Category varchar(max),
	Product_SubCat_Code int,
	Product_SubCategory varchar(max)
)

create table Dim_Store
(
	Store_Key int identity(1,1) primary key,
	Store_Type varchar(max)
)
...
(
	Fact_Tran_Key int identity(1,1) primary key,
	customer_key int FOREIGN KEY REFERENCES dim_customer(customer_key),
	Prod_Cat_SubCat_Key int FOREIGN KEY REFERENCES dim_product_cat_subcat(prod_cat_subcat_key),
	Store_Key int FOREIGN KEY REFERENCES dim_store(store_key),
	tran_date int FOREIGN KEY REFERENCES dimdate (datekey),
	Quantity int,
	rate float,
	tax float,
	total_amount float
